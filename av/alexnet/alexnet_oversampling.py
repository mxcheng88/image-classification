...
if __name__ == '__main__':
    ...
    for train, test in sfolder.split(images, labels):
        ...
        metrics= Metric() 
        cbks = [metrics, TensorBoard(log_dir='./alexnet_train_record/alexnet_oversampling/',                            histogram_freq=1),                LearningRateScheduler(scheduler)]
        ...
        plt.savefig('./alexnet_oversampling.png')
        #plt.show()
        print('\n')
        print()
        print (time.strftime('%Y-%m-%d %H:%M:%S',time.localtime()))
        print('The current running python file is:alexnet_oversampling.py,83:21,validation_data=(x_test, y_test),batchsize=8,epochs=150,categorical_crossentropy ,sgd')
        print('\n')   
        # evaluate the model
        scores = alexnet.evaluate(x_test, y_test, verbose=0)
        alexnet.save('./alexnet_train_record/alexnet_oversampling.h5')
