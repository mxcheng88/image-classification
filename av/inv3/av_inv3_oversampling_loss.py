...
def load_imagedata():
    ...
    av_positive_number = [4, 8, 22, 51, 56, 61, 80]
    image=[]
    label=[]
    p = 0
    x = (1024-768)/2
...
if __name__ == '__main__':
    ...
    for train, test in sfolder.split(images, labels):
        ...
        #cbks = [TensorBoard(log_dir='./av_train_record/av_inv3_{}/'.format(args.dataset),histogram_freq=1),LearningRateScheduler(scheduler)]   
        cbks = [metric, TensorBoard(log_dir='./inv3_train_record/av_inv3_customed_loss_oversampling/',                            histogram_freq=1),                LearningRateScheduler(scheduler)]
        ...
        print('The current running python file is:av_inv3_customed_loss_oversampling.py,83:21,validation_data=(x_test, y_test),batchsize=8,epochs=100,categorical_crossentropy ,sgd')
        print('\n')
        # evaluate the model
        scores = inv3.evaluate(x_test, y_test, verbose=0)
        inv3.save('./inv3_train_record/inv3_customed_loss_oversampling.h5')
