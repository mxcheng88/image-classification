...
if __name__ == '__main__':
    ...
    for train, test in sfolder.split(images, labels):
        ...
        cbks = [metric, TensorBoard(log_dir='./inv3_train_record/av_inv3_oversampling/',                            histogram_freq=1),                LearningRateScheduler(scheduler)]
        ...
        print('The current running python file is:av_inv3_oversampling.py,83:21,validation_data=(x_test, y_test),batchsize=8,epochs=100,categorical_crossentropy ,sgd,oversampling')
        print('\n')
        # evaluate the model
        scores = inv3.evaluate(x_test, y_test, verbose=0)
        inv3.save('./inv3_train_record/av_inv3_oversampling.h5')
