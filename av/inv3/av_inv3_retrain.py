...
if __name__ == '__main__':
    ...
    inv3 = Model(inputs=base_model.input, outputs=predictions)    
    ...
    for train, test in sfolder.split(images, labels):
        ...
        cbks = [metric, TensorBoard(log_dir='./inv3_train_record/av_inv3_retrain/',                            histogram_freq=1),                LearningRateScheduler(scheduler)]
        ...
        plt.title('Inv3 retrain model report')
        ...
        print('The current running python file is:av_inv3_retrain.py,validation_data=(x_test, y_test),batchsize=8,epochs=100,categorical_crossentropy ,sgd')
        print('\n')
        # evaluate the model
        scores = inv3.evaluate(x_test, y_test, verbose=0)
        inv3.save('./inv3_train_record/inv3_retrain.h5')
