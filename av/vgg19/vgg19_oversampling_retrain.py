...
if __name__ == '__main__':
    ...
    print("========================================")
    print("MODEL: VGG19 oversampling retrain")
    ...
    for train, test in sfolder.split(images, labels):
        ...
        plt.title('vgg19 model oversampling_retrain report')
        plt.ylabel('evaluation')
        plt.xlabel('epoch')
        plt.legend(['train_accuracy','train_loss', 'val_accuracy','val_loss','val_f1-score','val_precisions','val_recalls'], loc='lower right')
        plt.savefig('./vgg19_oversampling_retrain.png')
        #plt.show()
        print('\n')
        print (time.strftime('%Y-%m-%d %H:%M:%S',time.localtime()))
        print('The current running python file is:vgg19_oversampling_retrain.py,83:21,validation_data=(x_test, y_test),batchsize=8,epochs=100,sgd')
        print('\n') 
        # evaluate the model
