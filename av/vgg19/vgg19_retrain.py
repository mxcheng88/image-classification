...
if __name__ == '__main__':
    ...
    for train, test in sfolder.split(images, labels):
        ...
        cbks = [metric, TensorBoard(log_dir='./vgg19_train_record/av_vgg19_retrain/',histogram_freq=1),                LearningRateScheduler(scheduler)]
        ...
        plt.title('vgg19 model retrain report')
        plt.ylabel('evaluation')
        plt.xlabel('epoch')
        plt.legend(['train_accuracy','train_loss', 'val_accuracy','val_loss','val_f1-score','val_precisions','val_recalls'], loc='lower right')
        plt.savefig('./vgg19_retrain.png')
        #plt.show()
        print('\n')
        print()
        print (time.strftime('%Y-%m-%d %H:%M:%S',time.localtime()))
        print('The current running python file is:vgg19_retrain.py,83:21,validation_data=(x_test, y_test),batchsize=8,epochs=150,categorical_crossentropy,sgd')
        print('\n')   
        # evaluate the model
        scores = vgg19.evaluate(x_test, y_test, verbose=0)
        vgg19.save('./vgg19_train_record/av_vgg19_retrain.h5')
