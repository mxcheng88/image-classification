...
if __name__ == '__main__':
    ...
    print("========================================")
    print("MODEL: VGG19 customed_loss")
    ...
    #plot_model(vgg19,to_file='./av_train_record/vgg19.png',show_shapes=True)
    #print(vgg19.summary())
    print("== LOADING DATA... ==")
    # load data
    num_classes = 2
    ...
    for train, test in sfolder.split(images, labels):
        ...
        #cbks = [TensorBoard(log_dir='./av_train_record/av_vgg19_{}/'.format(args.dataset),histogram_freq=1),LearningRateScheduler(scheduler)]   
        metrics= Metric()
        cbks = [metrics, TensorBoard(log_dir='./vgg19_train_record/vgg19_customed_loss/',          histogram_freq=1),                LearningRateScheduler(scheduler)]
        ...
        plt.title('vgg19 model customed_loss report')
        plt.ylabel('evaluation')
        plt.xlabel('epoch')
        plt.legend(['train_accuracy','train_loss', 'val_accuracy','val_loss','val_f1-score','val_precisions','val_recalls'], loc='lower right')
        plt.savefig('./vgg19_customed_loss.png')
        #plt.show()
        print('\n')
        print (time.strftime('%Y-%m-%d %H:%M:%S',time.localtime()))
        print('The current running python file is:vgg19_customed_loss.py,83:21,validation_data=(x_test, y_test),batchsize=8,epochs=150,customed_loss,sgd')
        print('\n')       
        # evaluate the model
        scores = vgg19.evaluate(x_test, y_test, verbose=0)
        vgg19.save('./vgg19_train_record/av_vgg19_customed_loss.h5')
