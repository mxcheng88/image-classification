...
if __name__ == '__main__':
    ...
    for train, test in sfolder.split(images, labels):
        ...
        #senet.compile(loss='categorical_crossentropy', optimizer=sgd, metrics=['accuracy', precision, recall, f1score])
        #senet.compile(loss=customed_loss, optimizer=sgd, metrics=['accuracy', precision, recall, f1score])
        senet.compile(loss=customed_loss, optimizer=sgd, metrics=['accuracy'])
        # set callback
        cbks = [metrics,TensorBoard(log_dir='./senet_train_record/senet_oversampling_customed_loss/', histogram_freq=1),LearningRateScheduler(scheduler)]   
        #cbks = [metrics, TensorBoard(log_dir='./VVC_train_record/vvc_senet_{:d}_{}/'.format(layers, args.dataset), write_images=1,                            histogram_freq=1),                LearningRateScheduler(scheduler)]
        ...
        plt.title('senet model oversampling customed_loss report')
        plt.ylabel('evaluation')
        plt.xlabel('epoch')
        plt.legend(['train_accuracy','train_loss', 'val_accuracy','val_loss','val_f1-score','val_precisions','val_recalls'], loc='lower right')
        plt.savefig('./senet_oversampling_customed_loss.png')
        #plt.show()
        print('\n')
        print (time.strftime('%Y-%m-%d %H:%M:%S',time.localtime()))
        print('The current running python file is:senet_customed_loss_oversampling.py,83:21,validation_data=(x_test, y_test),batchsize=8,epochs=150,customed_loss ,sgd')
        print('\n') 
        # evaluate the model
        scores = senet.evaluate(x_test, y_test, verbose=0)
        senet.save('./senet_train_record/senet_customed_loss_oversampling.h5')
