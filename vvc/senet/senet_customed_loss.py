...
if __name__ == '__main__':
    ...
    #plot_model(senet,to_file='./VVC_train_record/senet.png',show_shapes=True)
    #print(senet.summary())
    print("== LOADING DATA... ==")
    # load data
    num_classes = 2
    ...
    for train, test in sfolder.split(images, labels):
        ...
        #cbks = [TensorBoard(log_dir='./VVC_train_record/vvc_senet_{}/'.format(args.dataset),histogram_freq=1),LearningRateScheduler(scheduler)]   
        metrics= Metric()
        cbks = [metrics, TensorBoard(log_dir='./senet_train_record/senet_customed_loss/',          histogram_freq=1),                LearningRateScheduler(scheduler)]
        ...
        plt.title('senet model customed_loss report')
        plt.ylabel('evaluation')
        plt.xlabel('epoch')
        plt.legend(['train_accuracy','train_loss', 'val_accuracy','val_loss','val_f1-score','val_precisions','val_recalls'], loc='lower right')
        plt.savefig('./senet_customed_loss.png')
        #plt.show()
        print('\n')
        print (time.strftime('%Y-%m-%d %H:%M:%S',time.localtime()))
        print('The current running python file is:senet_customed_loss.py,83:21,validation_data=(x_test, y_test),batchsize=8,epochs=150,customed_loss,sgd')
        print('\n')       
        # evaluate the model
        scores = senet.evaluate(x_test, y_test, verbose=0)
        senet.save('./senet_train_record/vvc_senet_customed_loss.h5')
