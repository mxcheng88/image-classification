...
from keras.layers import Conv2D, Dense, Input, add, Activation, GlobalAveragePooling2D,Dropout,Flatten,MaxPooling2D
...
if __name__ == '__main__':
    ...
    base_model=alexnet(input_tensor=input_tensor,weights='imagenet',include_top=False)    
    x = base_model.output
    ...
    for train, test in sfolder.split(images, labels):
        ...
        cbks = [metric, TensorBoard(log_dir='./alexnet_train_record/vvc_alexnet_retrain/',histogram_freq=1),                LearningRateScheduler(scheduler)]
        ...
        plt.title('alexnet model retrain report')
        plt.ylabel('evaluation')
        plt.xlabel('epoch')
        plt.legend(['train_accuracy','train_loss', 'val_accuracy','val_loss','val_f1-score','val_precisions','val_recalls'], loc='lower right')
        plt.savefig('./alexnet_retrain.png')
        #plt.show()
        print('\n')
        print()
        print (time.strftime('%Y-%m-%d %H:%M:%S',time.localtime()))
        print('The current running python file is:alexnet_retrain.py,83:21,validation_data=(x_test, y_test),batchsize=8,epochs=150,categorical_crossentropy,sgd')
        print('\n')   
        # evaluate the model
        scores = alexnet.evaluate(x_test, y_test, verbose=0)
        alexnet.save('./alexnet_train_record/vvc_alexnet_retrain.h5')
