...
parser.add_argument('-e', '--epochs', type=int, default=20, metavar='NUMBER',
...
num_classes = 2
img_rows, img_cols,img_channels = 768, 768,3
...
if __name__ == '__main__':
    ...
    '''
    alexnet = keras.models.load_model("/home/admin301/chengmx/_vvc/vvc_alexnet/alexnet_train_record/vvc_alexnet_customed_loss.h5",custom_objects={'customed_loss':customed_loss})
    # print model architecture if you need.
    #plot_model(alexnet,to_file='./VVC_train_record/alexnet.png',show_shapes=True)
    ...
    for train, test in sfolder.split(images, labels):
        ...
        #cbks = [TensorBoard(log_dir='./VVC_train_record/vvc_alexnet_{}/'.format(args.dataset),histogram_freq=1),LearningRateScheduler(scheduler)]   
        metrics= Metric()
        cbks = [metrics, TensorBoard(log_dir='./alexnet_train_record/2alexnet_customed_loss/',          histogram_freq=1),                LearningRateScheduler(scheduler)]
        ...
        plt.title('alexnet model customed_loss report')
        plt.ylabel('evaluation')
        plt.xlabel('epoch')
        plt.legend(['train_accuracy','train_loss', 'val_accuracy','val_loss','val_f1-score','val_precisions','val_recalls'], loc='lower right')
        plt.savefig('./2alexnet_customed_loss.png')
        #plt.show()
        print('\n')
        print (time.strftime('%Y-%m-%d %H:%M:%S',time.localtime()))
        print('The current running python file is:alexnet_customed_loss.py,83:21,validation_data=(x_test, y_test),batchsize=8,epochs=150,customed_loss,sgd')
        print('\n')       
        # evaluate the model
        scores = alexnet.evaluate(x_test, y_test, verbose=0)
        alexnet.save('./alexnet_train_record/2vvc_alexnet_customed_loss.h5')
