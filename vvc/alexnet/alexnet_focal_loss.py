...
if __name__ == '__main__':
    ...
    for train, test in sfolder.split(images, labels):
        ...
        alexnet.compile(loss=focal_loss, optimizer=sgd, metrics=['accuracy'])
        ...
        metrics= Metric()
        cbks = [metrics, TensorBoard(log_dir='./alexnet_train_record/alexnet_focal_loss/',          histogram_freq=1),                LearningRateScheduler(scheduler)]
        ...
        print('the network output£º'y_pred)
        ...
        plt.title('alexnet model focal_loss report')
        plt.ylabel('evaluation')
        plt.xlabel('epoch')
        plt.legend(['train_accuracy','train_loss', 'val_accuracy','val_loss','val_f1-score','val_precisions','val_recalls'], loc='lower right')
        plt.savefig('./alexnet_focal_loss.png')
        #plt.show()
        print('\n')
        print (time.strftime('%Y-%m-%d %H:%M:%S',time.localtime()))
        print('The current running python file is:alexnet_focal_loss.py,83:21,validation_data=(x_test, y_test),batchsize=8,epochs=150,focal_loss,sgd')
        print('\n')       
        # evaluate the model
        scores = alexnet.evaluate(x_test, y_test, verbose=0)
        alexnet.save('./alexnet_train_record/vvc_alexnet_focal_loss.h5')
