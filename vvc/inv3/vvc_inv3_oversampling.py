...
if __name__ == '__main__':
    ...
    for train, test in sfolder.split(images, labels):
        ...
        cbks = [metric, TensorBoard(log_dir='./VVC_train_record/vvc_inv3_oversampling/',                            histogram_freq=1),                LearningRateScheduler(scheduler)]
        ...
        plt.savefig('./inv3_oversampling.png')
        #plt.show()
        
        print('\n')
        print (time.strftime('%Y-%m-%d %H:%M:%S',time.localtime()))
        print('The current running python file is:vvc_inv3.py,83:21,validation_data=(x_test, y_test),batchsize=8,epochs=100,categorical_crossentropy ,sgd,oversampling')
        print('\n')
        # evaluate the model
        scores = inv3.evaluate(x_test, y_test, verbose=0)
        inv3.save('./VVC_train_record/vvc_inv3_oversampling.h5')
