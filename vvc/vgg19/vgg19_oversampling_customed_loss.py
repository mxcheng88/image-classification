...
if __name__ == '__main__':
    ...
    print("========================================")
    print("MODEL: VGG19 oversampling_customed_loss")
    ...
    print("== DONE! ==\n== BUILD MODEL... ==")
    # build network
    base_model=VGG19(weights=None,include_top=True,input_shape=(img_rows, img_cols, img_channels),classes=2)
    vgg19 = Model(inputs=[base_model.input], outputs=[base_model.output])      
    ...
    for train, test in sfolder.split(images, labels):
        ...
        #vgg19.compile(loss='categorical_crossentropy', optimizer=sgd, metrics=['accuracy', precision, recall, f1score])
        #vgg19.compile(loss=customed_loss, optimizer=sgd, metrics=['accuracy', precision, recall, f1score])
        vgg19.compile(loss=customed_loss, optimizer=sgd, metrics=['accuracy'])
        # set callback
        cbks = [metrics,TensorBoard(log_dir='./vgg19_train_record/vgg19_oversampling_customed_loss/', histogram_freq=1),LearningRateScheduler(scheduler)]   
        #cbks = [metrics, TensorBoard(log_dir='./VVC_train_record/vvc_vgg19_{:d}_{}/'.format(layers, args.dataset), write_images=1,                            histogram_freq=1),                LearningRateScheduler(scheduler)]
        ...
        # start training
        history = vgg19.fit_generator(datagen.flow(x_train, y_train, batch_size=batch_size),
        ...
        plt.title('vgg19 model oversampling customed_loss report')
        plt.ylabel('evaluation')
        plt.xlabel('epoch')
        plt.legend(['train_accuracy','train_loss', 'val_accuracy','val_loss','val_f1-score','val_precisions','val_recalls'], loc='lower right')
        plt.savefig('./vgg19_oversampling_customed_loss.png')
        #plt.show()
        print('\n')
        print (time.strftime('%Y-%m-%d %H:%M:%S',time.localtime()))
        print('The current running python file is:vgg19_customed_loss_oversampling.py,83:21,validation_data=(x_test, y_test),batchsize=8,epochs=150,customed_loss ,sgd')
        print('\n') 
        # evaluate the model
        scores = vgg19.evaluate(x_test, y_test, verbose=0)
        vgg19.save('./vgg19_train_record/vgg19_customed_loss_oversampling.h5')
        print("%s: %.2f%%" % (vgg19.metrics_names[1], scores[1] * 100))
