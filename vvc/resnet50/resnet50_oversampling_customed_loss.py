...
if __name__ == '__main__':
    ...
    print("========================================")
    print("MODEL: Residual Network 50 oversampling_customed_loss")
    ...
    print("== DONE! ==\n== BUILD MODEL... ==")
    # build network
    base_model=ResNet50(weights=None,include_top=True,input_shape=(img_rows, img_cols, img_channels),classes=2)
    resnet50 = Model(inputs=[base_model.input], outputs=[base_model.output])      
    ...
    for train, test in sfolder.split(images, labels):
        ...
        #resnet50.compile(loss=customed_loss, optimizer=sgd, metrics=['accuracy', precision, recall, f1score])
        resnet50.compile(loss=customed_loss, optimizer=sgd, metrics=['accuracy'])
        # set callback
        cbks = [metrics,TensorBoard(log_dir='./resnet50_train_record/vvc_resnet50_customed_loss_oversampling/', histogram_freq=1),LearningRateScheduler(scheduler)]   
        #cbks = [metrics, TensorBoard(log_dir='./VVC_train_record/vvc_resnet50_{:d}_{}/'.format(layers, args.dataset), write_images=1,                            histogram_freq=1),                LearningRateScheduler(scheduler)]
        ...
        plt.title('ResNet50 model customed_loss_oversampling report')
        plt.ylabel('evaluation')
        plt.xlabel('epoch')
        plt.legend(['train_accuracy','train_loss', 'val_accuracy','val_loss','val_f1-score','val_precisions','val_recalls'], loc='lower right')
        plt.savefig('./resnet50_customed_loss_oversampling.png')
        #plt.show()
        print('\n')
        print (time.strftime('%Y-%m-%d %H:%M:%S',time.localtime()))
        print('The current running python file is:resnet50_customed_loss_oversampling.py,83:21,validation_data=(x_test, y_test),batchsize=8,epochs=150,customed_loss ,sgd')
        print('\n') 
        # evaluate the model
        scores = resnet50.evaluate(x_test, y_test, verbose=0)
        resnet50.save('./resnet50_train_record/vvc_resnet50_customed_loss_oversampling.h5')
