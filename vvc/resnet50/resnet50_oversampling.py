...
if __name__ == '__main__':
    ...
    for train, test in sfolder.split(images, labels):
        ...
        metrics= Metric() 
        cbks = [metrics, TensorBoard(log_dir='./resnet50_train_record/vvc_resnet50_oversampling/',                            histogram_freq=1),                LearningRateScheduler(scheduler)]
        ...
        plt.title('ResNet50 model oversampling report')
        plt.ylabel('evaluation')
        plt.xlabel('epoch')
        plt.legend(['train_accuracy','train_loss', 'val_accuracy','val_loss','val_f1-score','val_precisions','val_recalls'], loc='lower right')
        plt.savefig('./resnet50_oversampling.png')
        #plt.show()
        print('\n')
        print()
        print (time.strftime('%Y-%m-%d %H:%M:%S',time.localtime()))
        print('The current running python file is:resnet50_oversampling.py,83:21,validation_data=(x_test, y_test),batchsize=8,epochs=150,categorical_crossentropy ,sgd')
        print('\n')   
        # evaluate the model
        scores = resnet50.evaluate(x_test, y_test, verbose=0)
        resnet50.save('./resnet50_train_record/vvc_resnet50_oversampling.h5')
