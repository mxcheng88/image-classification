...
if __name__ == '__main__':
    ...
    for train, test in sfolder.split(images, labels):
        ...
        cbks = [metrics, TensorBoard(log_dir='./resnet50_train_record/resnet50_{}/'.format(args.dataset),    histogram_freq=1),                LearningRateScheduler(scheduler)]
        ...
        plt.xlabel('epoch')
        plt.legend(['train_accuracy','train_loss', 'val_accuracy','val_loss', 'val_accuracy','val_loss','val_f1-score','val_precisions','val_recalls'], loc='lower right')
        plt.savefig('./resnet50.png')
        #plt.show()
        print('\n')
        print()
        print (time.strftime('%Y-%m-%d %H:%M:%S',time.localtime()))
        print('The current running python file is:resnet50.py,83:21,validation_data=(x_test, y_test),batchsize=8,epochs=150,categorical_crossentropy ,sgd')
        print('\n')       
        # evaluate the model
        scores = resnet50.evaluate(x_test, y_test, verbose=0)
        resnet50.save('./resnet50_train_record/resnet50_{}.h5'.format(args.dataset))
